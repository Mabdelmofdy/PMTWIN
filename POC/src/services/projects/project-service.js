...
(function() {
  ...
  async function createProject(projectData) {
    ...
    if (typeof PMTwinRBAC !== 'undefined') {
      const hasPermission = await PMTwinRBAC.canCurrentUserAccess('create_projects') ||
                           await PMTwinRBAC.canCurrentUserAccess('create_megaprojects');
      if (!hasPermission) {
        return { success: false, error: 'You do not have permission to create projects' };
      }
    }
    
    // Validate megaproject structure
    if (projectData.projectType === 'mega' || projectData.subProjects) {
      const validation = validateSubprojectStructure(projectData);
      if (!validation.valid) {
  ...
  // ============================================
  // Validate Subproject Structure
  // ============================================
  function validateSubprojectStructure(projectData) {
    // If it's a megaproject, it must have subprojects
    if (projectData.projectType === 'mega') {
      if (!projectData.subProjects || !Array.isArray(projectData.subProjects) || projectData.subProjects.length === 0) {
        return { valid: false, error: 'Megaprojects must have at least one subproject' };
      }
    }
    
    // Validate each subproject is complete
    if (projectData.subProjects && Array.isArray(projectData.subProjects)) {
      for (let i = 0; i < projectData.subProjects.length; i++) {
        const subproject = projectData.subProjects[i];
        
        if (typeof ProposalValidator !== 'undefined') {
          const validation = ProposalValidator.validateSubprojectCompleteness(subproject);
          if (!validation.valid) {
            return { valid: false, error: `Subproject ${i + 1}: ${validation.error}` };
          }
        } else {
          // Basic validation if validator not available
          if (!subproject.title || !subproject.description || !subproject.category) {
            return { valid: false, error: `Subproject ${i + 1} is incomplete. Missing required fields.` };
          }
        }
      }
    }
